...
CONF="${mount_point}/writable/etc/pacman.d/hooks/update-extlinux.hook"
mkdir -p ${mount_point}/writable/etc/pacman.d/hooks
cat << EOF2 > $CONF
[Trigger]
Operation = Upgrade
Type = Package
...
sed -i 's/^MODULES=(.*/MODULES=(ahci sd_mod ext4)/' ${mount_point}/writable/etc/mkinitcpio.conf
